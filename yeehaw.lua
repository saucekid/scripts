...
repeat wait() until game:IsLoaded() and game.PlaceId == 2317712696 and not _G.Executed1
_G.Executed1 = true
...
for i,v in next, getconstants(Global.UIHandler.GiveNotification) do
    if i < 15 and type(v) == "function" then
        setconstant(Global.UIHandler.GiveNotification, i, function() return "uwu" end)   --thx twisted
    end
end
...
spawn(function()
    while wait() do
        for i, v in next, weapons.realweapons do
            v.FanAccuracy = (settings.nospread or settings.aim.silentaim) and 1 or weapons.ogvalues[i].FanAccuracy
            v.ProjectileAccuracy = (settings.nospread or settings.aim.silentaim) and 1 or weapons.ogvalues[i].ProjectileAccuracye
            v.ReloadSpeed =  settings.instantreload and 1000 or weapons.ogvalues[i].ReloadSpeed
            v.LoadSpeed =  settings.instantreload and 1000 or weapons.ogvalues[i].LoadSpeed
            v.LoadEndSpeed = settings.instantreload and 1000 or weapons.ogvalues[i].LoadEndSpeed
        end
...
OldInitProjectiles = hookfunction(ProjectileHandlerModule.InitProjectiles, function(c, Value, Data, Other, Callback)
    if settings.nospread then
        Other.accuracy = Random.new():NextNumber(0.98, 1)
    end
...
aim = library.newsection({name = "Aimbot", tab = CheatsTab,side = "left", size = 275,})
    ...
    library.newslider({
	    ...
	    settings.aim.smoothness = 1 - num/10
	end
    })
...
discord = library.newsection({name = "Discord", tab = MiscTab,side = "right", size = 55,})
    library.newbutton({name = "Copy to Clipboard",section = discord,tab = MiscTab,callback = function() if setclipboard then setclipboard('https://discord.gg/DnyxZRwQh3') else print("DnyxZRwQh3") end end})

    library.newbutton({name = "Direct Join",section = discord,tab = MiscTab,callback = function()
        local json = {
            ["cmd"] = "INVITE_BROWSER",
            ["args"] = {
            ["code"] = "DnyxZRwQh3"
...
--settings
save = library.newsection({name = "Saving", tab = MiscTab,side = "left", size = 55,})
    library.newtoggle({
	    name = "Autosave",
    	section = save,
	    tab = MiscTab,
